...
function normalizeEnvironment(value: unknown): TestRunnerEnvironment | null {
  const v = String(value ?? "")
    .trim()
    .toLowerCase();
  if (v === "dev" || v === "tst") return v;
  return null;
}

function normalizeSuiteTag(value: unknown): string | null {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const body = (req.body ?? {}) as Partial<WorkflowDispatchRequest>;
    const environment = normalizeEnvironment(body.environment);
    const suiteTag = normalizeSuiteTag(body.suiteTag);
    ...
    await dispatchWorkflow(githubConfig, { environment, suiteTag });

    const run = await findWorkflowRun(githubConfig, startedAtMs);
    if (!run) {
      return res.status(202).json({
        success: false,
        message: "Workflow dispatched but run link is not ready yet. Please check GitHub Actions.",
      });
    }
